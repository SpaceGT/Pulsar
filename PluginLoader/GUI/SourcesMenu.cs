...
{
    internal class SourcesMenu : PluginScreen
    {
        ...
        private List<ModConfig> ModSources;

        private List<RemoteHubConfig> RemoteHubSources;
        private List<LocalHubConfig> LocalHubSources;

        private List<RemotePluginConfig> RemotePluginSources;
        private List<LocalPluginConfig> LocalPluginSources;

        private Dictionary<object, bool> EnabledSourceChanges;
        ...
        public SourcesMenu(SourcesConfig sources)
            : base(size: new Vector2(1, 0.9f))
        {
            Sources = sources;
            EnabledSourceChanges = [];
            RemoteHubSources = [.. sources.RemoteHubSources];
            LocalHubSources = [.. sources.LocalHubSources];
            RemotePluginSources = [.. sources.RemotePluginSources];
            LocalPluginSources = [.. sources.LocalPluginSources];
            ModSources = [.. sources.ModSources];
        }
        ...
        public override void RecreateControls(bool constructor)
        {
            ...
            MyGuiControlButton btnRefresh = new MyGuiControlButton(
            ...
            MyGuiControlButton btnApply = new MyGuiControlButton(
            ...
            MyGuiControlButton btnCancel = new MyGuiControlButton(
                position: new Vector2(
                    bottomMid.X + 0.5f * btnRefresh.Size.X + GuiSpacing,
                    bottomMid.Y - GuiSpacing
                ),
            ...
            // Column 2
            grid.Add(
                new MyGuiControlLabel(text: "Plugins"),
                MyAlignH.Center,
                MyAlignV.Bottom,
                0,
                1
        ...
        private void OpenSourceView(object source)
        {
            ...
            ViewSourceMenu view = new ViewSourceMenu(source, RemoveSource);
        ...
        private void CreateSourcesPanel(MyGuiControlParent parent, PluginType type)
        {
            ...
            MyGuiControlButton template = new MyGuiControlButton(
            ...
            {
                MyGuiControlButton remoteHubAdd = new MyGuiControlButton(
                    visualStyle: VRage.Game.MyGuiControlButtonStyleEnum.SquareSmall,
                ...
                remoteHubAdd.Position = new Vector2(
                ...
                MyGuiControlButton localHubAdd = new MyGuiControlButton(
                    visualStyle: VRage.Game.MyGuiControlButtonStyleEnum.SquareSmall,
                    toolTip: "Add Local Hub",
                    originAlign: VRage.Utils.MyGuiDrawAlignEnum.HORISONTAL_RIGHT_AND_VERTICAL_TOP,
                    onButtonClick: AddLocalHub
                );
                localHubAdd.Position = new Vector2(
                    -topLeft.X,
                    topLeft.Y + list.Size.Y + ButtonSpacing
                );
            ...
            {
                MyGuiControlButton remotePluginAdd = new MyGuiControlButton(
                    visualStyle: VRage.Game.MyGuiControlButtonStyleEnum.SquareSmall,
                    toolTip: "Add Remote Plugin",
                    originAlign: VRage.Utils.MyGuiDrawAlignEnum.HORISONTAL_RIGHT_AND_VERTICAL_TOP,
                    onButtonClick: (x) =>
                        MyGuiSandbox.AddScreen(new AddRemotePlugin(AddRemotePlugin))
                );
                remotePluginAdd.Position = new Vector2(
                ...
                MyGuiControlButton folderPluginAdd = new MyGuiControlButton(
                    visualStyle: VRage.Game.MyGuiControlButtonStyleEnum.SquareSmall,
                    toolTip: "Add Development Folder",
                    originAlign: VRage.Utils.MyGuiDrawAlignEnum.HORISONTAL_RIGHT_AND_VERTICAL_TOP,
                    onButtonClick: AddDevelopmentFolder
                );
                folderPluginAdd.Position = new Vector2(
                    -topLeft.X - template.Size.X - ButtonSpacing,
                    topLeft.Y + list.Size.Y + ButtonSpacing
                );
                ...
                MyGuiControlButton copmiledPluginAdd = new MyGuiControlButton(
                    visualStyle: VRage.Game.MyGuiControlButtonStyleEnum.SquareSmall,
                    toolTip: "Add Precompiled Plugin",
                    originAlign: VRage.Utils.MyGuiDrawAlignEnum.HORISONTAL_RIGHT_AND_VERTICAL_TOP,
                    onButtonClick: AddLocalFile
                );
                copmiledPluginAdd.Position = new Vector2(
                    -topLeft.X,
                    topLeft.Y + list.Size.Y + ButtonSpacing
                );
            ...
            {
                MyGuiControlButton modAdd = new MyGuiControlButton(
                    visualStyle: VRage.Game.MyGuiControlButtonStyleEnum.SquareSmall,
                    toolTip: "Add Mod",
                    originAlign: VRage.Utils.MyGuiDrawAlignEnum.HORISONTAL_RIGHT_AND_VERTICAL_TOP,
                    onButtonClick: (x) => MyGuiSandbox.AddScreen(new AddMod(AddMod))
                );
                modAdd.Position = new Vector2(-topLeft.X, topLeft.Y + list.Size.Y + ButtonSpacing);
        ...
        )
        {
            ...
            {
                list.ColumnsCount = 2;
                list.SetCustomColumnWidths(new[] { 0.9f, 0.1f });
                list.SetColumnName(0, new StringBuilder("Name"));
            }
            else
            {
                list.ColumnsCount = 3;
                list.SetCustomColumnWidths(new[] { 0.45f, 0.45f, 0.1f });
                list.SetColumnName(0, new StringBuilder("Name"));
        ...
        private void MakeHubRows(MyGuiControlTable list)
        {
            foreach (RemoteHubConfig source in RemoteHubSources)
            {
                ...
                row.AddCell(new MyGuiControlTable.Cell(Tools.Tools.DateToString(source.LastCheck)));
        ...
        private void MakePluginRows(MyGuiControlTable list)
        {
            ...
            foreach (LocalPluginConfig source in LocalPluginSources)
            {
                ...
                enabledCheckbox.IsCheckedChanged += (x) =>
                    source.Enabled = enabledCheckbox.IsChecked;
                enabledCell.Control = enabledCheckbox;
                list.Controls.Add(enabledCheckbox);
                row.AddCell(enabledCell);
                    ...
                    if (exists)
                    {
                        MyGuiSandbox.AddScreen(
                            MyGuiSandbox.CreateMessageBox(
                                ...
                                messageCaption: new StringBuilder("Plugin Loader")
                            )
                        );
                        return;
                    }

                    LocalHubConfig hub = new LocalHubConfig()
                    {
        ...
        {
            MyGuiSoundManager.PlaySound(GuiSounds.MouseClick);
            LocalFolderPlugin.PromptFolder(
                (file) =>
                {
                    DirectoryInfo folder = Directory.GetParent(file);
                    LocalPluginConfig plugin = new LocalPluginConfig()
                    {
                        Name = folder.Name,
