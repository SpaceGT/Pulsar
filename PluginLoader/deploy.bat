...
echo Switching to "Bin64\Plugins\"

REM Copy loader into plugin directory
echo Copying "Loader.dll"

for /l %%i in (1, 1, 10) do (
    copy /y /b "%SOURCE%\Loader.dll" "%PLUGIN_DIR%\loader.dll" >NUL 2>&1
...
echo Could not copy "Loader.dll".
exit /b 1

:BREAK_LOOP

REM Copy the Application Config file
echo Copying "app.config"
copy /y /b "%CONFIG%" "%PLUGIN_DIR%\loader.dll.config" >NUL 2>&1
...
echo Copying "Microsoft.CodeAnalysis.CSharp.dll"
...
copy /y /b "%SOURCE%\System.Runtime.CompilerServices.Unsafe.dll" "%DEPENDENCY_DIR%\" >NUL 2>&1
